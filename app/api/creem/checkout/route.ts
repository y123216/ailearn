...
export async function POST(req: Request) {
  ...
  const productId = (parsed && typeof parsed === "object" && "productId" in parsed ? (parsed as { productId?: string }).productId : undefined) || process.env.NEXT_PUBLIC_CREEM_PRODUCT_ID;
  if (!productId) {
    return NextResponse.json({ error: "缺少产品ID", failureType: "code" }, { status: 400 });
  }
  const base = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:10000";
  const successUrl = `${base}/growth/learning?paid=1`;
  try {
    ...
    if (!resp.ok) {
      ...
      return NextResponse.json({ error: data?.error || "创建失败", failureType }, { status });
    }
